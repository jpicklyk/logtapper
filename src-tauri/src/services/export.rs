...
//! `.lts` archive — and the file leaves the tool, so the decision is
//! [`policy::should_anonymize_for`]`(ctx, `[`Pathway::External`]`)`, made once
//! per export for every session in it:
//!
//! - **`Ui`**: redacted under anonymizer mode `All` and `External` (the
//!   default), raw only under `None`. There is no per-export checkbox any
//!   more — the mode is the one switch, and the Export dialog reports it.
//! - **`Agent`**: redacted unless the mode is `None` or the user persisted the
//!   `agent_raw_access` opt-out — exactly as every other raw-line route. An
//!   agent exporting every open session is the widest version of the leak
//!   that gate exists to close, so nothing in the agent's own request body
//!   can influence it.
//!
//! Once redaction applies, the mechanism is [`policy::anonymize_session_lines`],
//! reusing the session's cached `LogAnonymizer` so token numbering matches the
//! viewer (under `All`) and any later clipboard copy — see
//! [`anonymize_lines_to_bytes`]. The decision is also recorded in the
//! archive's manifest (`LtsManifest::anonymized`) so a re-import can badge
//! the session instead of passing redacted logs off as raw.
...
/// Redact every line in `lines` for `session_id` through the anonymizer
/// mechanism ([`policy::anonymize_session_lines`] — one lock for the whole
/// session, the session's cached `LogAnonymizer` so token numbering is
/// stable) and reassemble into the exact byte layout `write_lts` expects for
/// a session's `source_bytes`: one record per line, `\n`-terminated, in
/// original order.
///
/// Unconditional — only ever called once `should_anonymize_for(External)`
/// has decided redaction applies. It does not truncate: export is never
/// subject to the bridge's 500-char response cap.
fn anonymize_lines_to_bytes(ctx: &ServiceCtx, session_id: &str, mut lines: Vec<String>) -> Vec<u8> {
...
pub async fn run(ctx: ServiceCtx, options: ExportAllOptions) -> Result<(), ServiceError> {
    ...
    enum SourceRef {
        ...
        /// redacted. Redacted (via [`anonymize_lines_to_bytes`]) once the
        /// `sessions` lock has been dropped — see step 2 below — instead of
        /// being written raw.
        RawLines(Vec<String>),
    }
    ...
    // — `policy::anonymize_session_lines` acquires `anonymizer_config` /
    // `mcp_anonymizers` itself, and nesting those under `sessions` would
    // violate the lock-ordering discipline `services/mod.rs` documents. The
    // decision covers every session in this export: the file leaves the
    // tool, so this is the `External` pathway — see the module doc comment.
    ...
    let write_result = tokio::task::spawn_blocking(move || {
        crate::workspace::lts::write_lts_with(
            &dest_for_write,
            &lts_sessions,
            &processor_yamls,
            &editor_tabs,
...
mod tests {
    ...
    // ── the export decision (mode × caller, External pathway) ────────────
    //
    ...
    // no anonymizer call anywhere in the file. The per-export checkbox that
    // first fixed it is gone; export is now `should_anonymize_for(External)`
    // — `Ui` redacted under All/External, raw under None; an `Agent` as
    // every other raw-line route. The full table lives in `policy.rs`; these
    // pin the export's use of it.

    #[test]
    fn export_decision_ui_follows_the_mode() {
        let (ctx, _t1) = test_ctx().build();
        assert!(policy::should_anonymize_for(&ctx, Pathway::External), "External (default): a Ui export is redacted");
        let (ctx, _t2) = test_ctx().anonymizer_mode(AnonymizerMode::All).build();
        assert!(policy::should_anonymize_for(&ctx, Pathway::External));
        let (ctx, _t3) = test_ctx().anonymizer_mode(AnonymizerMode::None).build();
        assert!(!policy::should_anonymize_for(&ctx, Pathway::External), "None: raw, exports included");
    }

    #[test]
    fn export_decision_agent_is_redacted_unless_none_or_raw_access() {
        let (ctx, _t1) = test_ctx().agent("mcp").build();
        assert!(policy::should_anonymize_for(&ctx, Pathway::External));
        let (ctx, _t2) = test_ctx().agent("mcp").agent_raw_access(true).build();
        assert!(!policy::should_anonymize_for(&ctx, Pathway::External));
        let (ctx, _t3) = test_ctx().agent("mcp").anonymizer_mode(AnonymizerMode::None).build();
        assert!(!policy::should_anonymize_for(&ctx, Pathway::External));
    }

    // ── anonymize_lines_to_bytes (the mechanism) ─────────────────────────
    //
    // Unconditional — only ever invoked once the decision above has said
    // redaction applies (see `run`'s `SourceRef::RawLines` branch). No
    // caller dimension here.
    ...
    /// The source text of every `source/` entry in the archive, concatenated,
    /// plus the manifest's `anonymized` flag.
    fn read_archive(dest: &std::path::Path) -> (String, bool) {
        let bytes = std::fs::read(dest).unwrap();
        let mut zip = zip::ZipArchive::new(std::io::Cursor::new(bytes)).unwrap();
        ...
        assert!(found_source, "the archive must contain a source entry");
        let manifest = crate::workspace::lts::read_lts(dest).expect("read_lts").manifest;
        (text, manifest.anonymized)
    }

    fn options_for(dest: &std::path::Path) -> ExportAllOptions {
        ExportAllOptions {
            dest_path: dest.to_string_lossy().to_string(),
            include_bookmarks: false,
            include_analyses: false,
    ...
    /// A `Ui` export is an `External` pathway: redacted under the default
    /// `External` mode and under `All`, with the manifest saying so.
    #[tokio::test]
    async fn run_ui_redacts_the_archive_under_external_and_all_and_marks_the_manifest() {
        for mode in [AnonymizerMode::External, AnonymizerMode::All] {
            let (ctx, tmp) = test_ctx().anonymizer_mode(mode).with_pii_session("s1", 3).build();
            let dest = tmp.path().join("ui-anon.lts");
            run(ctx, options_for(&dest)).await.expect("ui export should succeed");

            let (text, anonymized) = read_archive(&dest);
            assert!(!text.contains("@example.com"), "{mode:?}: raw PII leaked into a Ui export: {text}");
            assert!(text.contains("<EMAIL-"), "{mode:?}: expected anonymizer tokens: {text}");
            assert!(anonymized, "{mode:?}: the manifest must record the redaction");
        }
    }

    /// `None` means none — a `Ui` export writes the capture as-is and the
    /// manifest says raw.
    #[tokio::test]
    async fn run_ui_writes_raw_content_under_none() {
        let (ctx, tmp) = test_ctx().anonymizer_mode(AnonymizerMode::None).with_pii_session("s1", 3).build();
        let dest = tmp.path().join("ui-raw.lts");
        run(ctx, options_for(&dest)).await.expect("ui export should succeed");

        let (text, anonymized) = read_archive(&dest);
        assert!(text.contains("@example.com"), "None: a Ui export must stay raw: {text}");
        assert!(!anonymized);
    }

    /// An agent export follows the same rule as its bridge reads: raw only
    /// under `None` or with the raw-access opt-out. Nothing in the request
    /// body can influence it (the old `anonymize` field is gone; an unknown
    /// key in an old client's body is ignored by serde).
    #[tokio::test]
    async fn run_agent_is_raw_only_under_none_or_raw_access() {
        for (mode, raw_access, expect_raw) in [
            ...
            let (ctx, tmp) = test_ctx()
                .agent("mcp")
                .anonymizer_mode(mode)
                .agent_raw_access(raw_access)
                .with_pii_session("s1", 3)
                .build();
            let out_dir = tmp.path().join("allowed-out");
            std::fs::create_dir_all(&out_dir).unwrap();
            ...
            let dest = out_dir.join("agent.lts");
            run(ctx, options_for(&dest)).await.expect("agent export inside the allowlist should succeed");

            let (text, anonymized) = read_archive(&dest);
            assert_eq!(
                text.contains("@example.com"),
                expect_raw,
                "{mode:?}/raw_access={raw_access}: {text}"
            );
            assert_eq!(anonymized, !expect_raw, "{mode:?}/raw_access={raw_access}: manifest flag");
        }
    }
}
