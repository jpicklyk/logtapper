...
async fn route_table_probe_every_route_resolves_through_the_live_router() {
    ...
    // time WP-T2 landed, 42 after WP-12 appended its 3 settings routes, 46
    // after WP-10 appended its 4 timeline/export routes) — a drift here means
    // BOTH tests need updating, which is the point: it forces a route
    // addition/removal to touch this file. Other Wave-2 packages append their
    // own routes concurrently in sibling worktrees, so this exact number is
    // expected to hit a merge conflict when those branches combine — resolve
    // it by summing every package's additions, not by picking one side.
    assert_eq!(
        routes.len(),
        46,
