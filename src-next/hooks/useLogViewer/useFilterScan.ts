...
export function useFilterScan(cacheManager: CacheController, refs: SharedLogViewerRefs): FilterScanResult {
  ...
  const setStreamFilter = useCallback(async (expr: string) => {
    ...
    if (sess) setSessionFilter(sess.sessionId, { streamFilter: expr });
    ...
    try {
      ...
      if (sess) setSessionFilter(sess.sessionId, { filterParseError: null });
    } catch (e) {
    ...
    if (!sess) return;
    cancelActiveBackendFilter();
    setSessionFilter(sess.sessionId, { filterScanning: true });
    ...
    if (backendFilter) {
      ...
      // Fetches all lines via getLines IPC (reads from LogSource, including
      // SpillFile for evicted streaming lines) — does NOT scan the cache.
      const BATCH = 20000;
  ...
  }, [setSessionFilter, refs.sessionRef]);
